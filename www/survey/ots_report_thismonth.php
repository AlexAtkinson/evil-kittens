...
$query="SELECT * FROM offtech.ot_survey WHERE Month(os_date)=Month(Now()) AND Year(os_date)=Year(Now())ORDER BY os_date;";
$result = mysql_query($query);
$result_q1 = mysql_query("SELECT avg(os_question_1) FROM offtech.ot_survey WHERE Month(os_date)=Month(Now()) AND Year(os_date)=Year(Now())ORDER BY os_date;");
while ($q1_avger = mysql_fetch_array ($result_q1)) {$q1_avg = $q1_avger[0];}
$result_q2 = mysql_query("SELECT avg(os_question_2) FROM offtech.ot_survey WHERE Month(os_date)=Month(Now()) AND Year(os_date)=Year(Now())ORDER BY os_date;");
while ($q2_avger = mysql_fetch_array ($result_q2)) {$q2_avg = $q2_avger[0];}
$result_q3 = mysql_query("SELECT avg(os_question_3) FROM offtech.ot_survey WHERE Month(os_date)=Month(Now()) AND Year(os_date)=Year(Now())ORDER BY os_date;");
while ($q3_avger = mysql_fetch_array ($result_q3)) {$q3_avg = $q3_avger[0];}
$result_q4 = mysql_query("SELECT avg(os_question_4) FROM offtech.ot_survey WHERE Month(os_date)=Month(Now()) AND Year(os_date)=Year(Now())ORDER BY os_date;");
while ($q4_avger = mysql_fetch_array ($result_q4)) {$q4_avg = $q4_avger[0];}
$result_q5 = mysql_query("SELECT avg(os_question_5) FROM offtech.ot_survey WHERE Month(os_date)=Month(Now()) AND Year(os_date)=Year(Now())ORDER BY os_date;");
while ($q5_avger = mysql_fetch_array ($result_q5)) {$q5_avg = $q5_avger[0];}
$result_q6 = mysql_query("SELECT avg(os_question_6) FROM offtech.ot_survey WHERE Month(os_date)=Month(Now()) AND Year(os_date)=Year(Now())ORDER BY os_date;");
while ($q6_avger = mysql_fetch_array ($result_q6)) {$q6_avg = $q6_avger[0];}
$result_overall = mysql_query("SELECT avg(os_question_6) FROM offtech.ot_survey WHERE Month(os_date)=Month(Now()) AND Year(os_date)=Year(Now())ORDER BY os_date;");
...
echo "<table border=\"0\">";
echo "<tr><td><table border=\"3\"><tr><td colspan=10 bgcolor=bcc62e align=center><font size=4 style=bold>Office Technologies Satisfaction Survey Results - ";
echo date("F");
echo " ";
echo date("Y");
...
echo "<tr>
<td align=left>$q1_avg</strong></td><td align=left>$q2_avg</td>
<td align=left>$q3_avg</strong></td><td align=left>$q4_avg</td>
<td align=left>$q5_avg</strong></td><td align=left>$q6_avg</td>
